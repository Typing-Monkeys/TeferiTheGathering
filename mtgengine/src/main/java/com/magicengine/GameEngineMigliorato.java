...
public class GameEngineMigliorato extends Thread {
	// CLASS VARIABLES
	...
	public GameEngineMigliorato(Socket s) {
		this.clientSocket = s;
		this.buffer = new char[1024];
		GameEngineMigliorato.gson = new Gson();
	...
	void initPlayer(String json) {
		...
		// AGGIUNTA
		Map<Integer, String> carteJson = new HashMap<Integer, String>(); 
		//FINE
		for (int i : deck) {
			String jsonRicevuto = carteJson.get(i);
			if(jsonRicevuto == null) {
				try {
					Runtime.getRuntime().exec("clear");
					System.out.println("______PATH_____ " + System.getProperty("user.dir"));
					System.out.println("______PATH_____ ");
					
					URL url = new URL("http://mtg.dmi.unipg.it/mancini/rest-server/api/imgFromCardid/" + i);
					Scanner scan = new Scanner(url.openStream());
					jsonRicevuto = scan.nextLine();
					carteJson.put(i, jsonRicevuto);
					receivedPlayer.getDeck().add(new CardMigliorato(jsonRicevuto, game.giveMeMyId(),i));
					scan.close();
				}catch(Exception e) {
		...
		/*for (int i = 0; i < deck.length; i++) {
			if (i < 20) {
				receivedPlayer.getDeck().add(new Card(900, game.giveMeMyId()));
			} else {
				receivedPlayer.getDeck().add(new Card(1065, game.giveMeMyId()));
			}
		}*/
		...
		try {
			...
			while (true) {
				...
				if (n != -1) {
					...
					if (received != null) {
						System.out.println("ricevuto attempt");
					...
					if (received != null) {
						...
						for(int i=0; i < game.getPriorityOrder().get(idappoggio).getHand().size(); i++){
					        ...
					        if (card.getId() == idCard){
						        	//variabile d'appoggio per swappare la carta 
						        	game.getPriorityOrder().get(idappoggio).getHand().remove(i);
						        	game.getStack().add(new Spell(card,card.getMagicTargetId(),idPlayr,game.getIdStackCounter()));
						        	break;
					        }
					    } 
